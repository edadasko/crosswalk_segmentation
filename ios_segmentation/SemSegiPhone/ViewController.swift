...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    func setupVisionModel() {
        guard let model = try? VNCoreMLModel(for: espnet_model.model) else{
...
class ColorMasker: CIFilter
{
    ...
    let colormapKernel = CIColorKernel(source:
        "kernel vec4 colorMasker(__sample gray)" +
            "{" +
            " if (gray.r < 0.5f) {return vec4(0.0, 0.0, 0.0, 1.0);}" +
            "   return vec4(gray.r, 0, 0, 1.0);" +
        "}"
    )
